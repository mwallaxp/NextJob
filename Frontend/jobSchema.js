...
export const frontendJobSchema = z.object({
  ...
  requirements: z.string({ required_error: "Requirements are required" }).min(10, "Requirements are required"),
  ...
  experienceLevel: z.coerce.number({ invalid_type_error: "Experience must be a number" }).int().nonnegative("Experience cannot be negative"),
...
export const jobDetailsStepSchema = frontendJobSchema.pick({
  title: true, description: true, requirements: true, salary: true,
  location: true, jobType: true, experienceLevel: true, position: true,
  skills: true,
});
